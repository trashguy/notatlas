...
| ✓ multi-commit 2026-05-12 | M10: gpu-driven-instancing | Mesh palette + SSBO-driven instancing (`15ec0cd`) → collapsed to `vkCmdDrawIndexedIndirect` (`f22d262`) → synthetic gate harness + clearance (`0e717aa`). Gate result on RX 9070 XT @ 5045 instances × 20 piece types, MAILBOX, 10 s: avg 1.93 ms, p99 2.95 ms, max 4.43 ms, ~518 fps. ~8.6× headroom on the 16.67 ms budget. M10.3 GPU compute culling intentionally deferred — at <2 ms avg the cull cost would dwarf savings; revisit if M11 piece geometry breaks budget. |
