...
| ◐ partial | Combat slice | One sloop with cannons (✓ ship-sim cannon fire end-to-end via TCP). Free-agent player capsule + board/disembark with lever-arm velocity inheritance (✓ `3798f72`). Sails (▢ rigging system not yet implemented; thrust force model is a placeholder). Planks / hull damage (▢). AI sloop opponent (▢ — needs `ai-sim` service; design ratified in [09-ai-sim.md](09-ai-sim.md), see §14 for implementation order). Closes when a sloop can sail wind-driven, take damage, and an AI opponent fights back. |
