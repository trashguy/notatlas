...
| ◐ partial | Combat slice | One sloop with cannons (✓ ship-sim cannon fire end-to-end via TCP). Free-agent player capsule + board/disembark with lever-arm velocity inheritance (✓ `3798f72`). Sails (▢ rigging system not yet implemented; thrust force model is a placeholder). Planks / hull damage (▢). AI sloop opponent (✓ `33d4064`+`1bffaf0` 2026-05-01: ai-sim service ships at 20 Hz with BT runtime + Lua leaves + perception ctx; AI ship#3 engages broadside at 180 m and fires every 4 s — verified end-to-end against ship-sim). Closes when a sloop can sail wind-driven and take damage. |
