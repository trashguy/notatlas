...
// M14.2c textured-cube fragment shader. Same lit-then-fogged shape as
// box.frag, but the base color comes from a sampled albedo texture
// instead of a pushed RGB. Tint (push-constant) modulates over the
// sampled value so the same texture can be reused with per-instance
// color shifts.
//
// Atmosphere helper + ACES tonemap match box.frag verbatim — keeping
// them inline (no #include) is the same trade box.frag made.
...
// New for M14: combined image sampler at set=0 binding=1.
// SPIR-V reflection (spirv_reflect.zig) picks this up automatically.
...
void main() {
    ...
    vec3 lit = albedo * (0.25 + 0.85 * n_dot_l) + sky_ambient * 0.05;
