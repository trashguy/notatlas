...
// M11.1 merged-anchorage fragment shader. Identical lighting + fog model
// to instanced.frag (and therefore box.frag) so far-LOD merged geometry
// matches near-LOD instanced geometry byte-for-byte in steady state.
// Kept as a separate file for the same glslc-include-path reason.
...
// Fixed reference direction — never sample fog by view_dir on opaque
// geometry. See memory `feedback_fixed_fog_direction.md`.
